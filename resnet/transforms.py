...
class ScaleJitter(nn.Module):
    ...
    Args:
        target_size (tuple of ints): The target size for the transform provided in (height, width) format.
    ...
    def forward(self, img, target=None):
        ...
        if pad_bottom != 0 or pad_right != 0:
            ...
            if isinstance(img, torch.Tensor):
                fill = torch.tensor(fill, dtype=img.dtype, device=img.device).item()
