...
def image_predict(output_dir, frames_output_dir, visual_prompts, debug=True):
    predictor = SAM2ImagePredictor(
        build_sam2(model_cfg, checkpoint, device=torch.device("cuda"))
    )
...
def propagate_and_save_masks(predictor, state, batch_prompts, frame_idx_offset, debug):
    ...
    for batch_frame_idx, _, masks in predictor.propagate_in_video(state):
        ...
        if len(batch_prompts[batch_frame_idx]) > 0:
            for point in batch_prompts[batch_frame_idx]:
                if point is None or mask[point[1], point[0], 0] != 1:
                    print(
                        f"Warning: Point {point} in frame {curr_idx} is not covered by the mask."
                    )
        ...
        if not np.any(mask):
            print(
                f"No valid masks generated for frame {curr_idx}. Skipping this frame."
            )
            continue
        # Image path
        if append:
        ...
        if debug:
            ...
            if image is None:
                print(
                    f"Warning: Unable to read image at {image_path}. Skipping this frame."
                )
                continue
            overlay = image.copy()
...
def process_video_frames(predictor, state, batch_prompts, frame_idx, debug):
    i = 0
    while i < len(batch_prompts) and (
        batch_prompts[i] is None or len(batch_prompts[i]) == 0
    ):
        i += 1
...
if __name__ == "__main__":
    ...
    frame_files = sorted(
        [
            f
            for f in os.listdir(frames_source_dir)
    ...
    if append:
        ...
        existing_files = sorted(
            [
                f
                for f in os.listdir(frames_output_dir)
                if os.path.isfile(os.path.join(frames_output_dir, f))
                and f.endswith(".jpg")
            ]
        )
        if existing_files:
    ...
    while frame_idx < len(frame_files):
        ...
        frame_idx += process_video_frames(
            predictor, state, batch_prompts, frame_idx, debug
        )
        del state
